...
/obj/item/weapon/cartridge/proc/create_NanoUI_values(mob/user as mob)
	...
	if(mode==43 || mode==433)
		...
		// Add the power in the network to the list, truncating any fractional parts
		// And allowing up to 999,999,999W before switching to scientific notation.
		values["poweravail"] = num2text(round(powmonitor.powernet.avail),10)
		values["powerload"] = num2text(round(powmonitor.powernet.viewload),10)
